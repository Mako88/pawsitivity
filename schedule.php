...
<?php
    ...
    if(empty($_POST)) {
        ...
        if(!empty($pets)) {
            ...
            echo '<label for="pet">Select which pets you would like to schedule: </label><br />';
            foreach($pets as $pet) {
                echo '<label for="' . $pet['ID'] . '">' . $pet['Name'] . '</label>';
                echo '<input type="checkbox" name="pet[]" id="' . $pet['ID'] . '" value="' . $pet['ID'] . '" /><br />';
            }
    ...
    else if($_SESSION['page'] == 2 || !empty($_GET['pet'])) {
        
        if(sizeof($_SESSION['info']['petids']) - 1 == sizeof($_SESSION['info']['pets'])) {
            $_SESSION['page'] = 3;
        }
        
        // First time through
        if(!empty($_GET['pet']) || !empty($_POST['pet'])) {
            $_SESSION['info'] = array();
            $_SESSION['info']['pets'] = array();
            $_SESSION['info']['petids'] = array();
            $index = 0;
            ...
            if(!empty($_GET['pet'])) {
                ...
                if(!is_array($_POST['pet'])) {
                    $_SESSION['info']['petids'][0] = $_POST['pet'];
                }
                else {
                    for($i = 0; $i < sizeof($_POST['pet']); $i++) {
                        $_SESSION['info']['petids'][$i] = $_POST['pet'][$i];
                    }
                }
                $petid = $_SESSION['info']['petids'][0];
            }
        }
        else {
            $index = sizeof($_SESSION['info']['pets']);
            $petid = $_SESSION['info']['petids'][$index];
            
            if($_POST['package'] == 1 || $_POST['package'] == 2 || $_POST['package'] == 3 || $_POST['package'] == 4) {
                $_SESSION['info']['pets'][$index]['package'] = $_POST['package'];
            }
        
            if(!empty($_POST['services']) && is_array($_POST['services'])) {
                $_SESSION['info']['pets'][$index]['services'] = $_POST['services'];
            }
        }
        ...
        if(!empty($pet)) {
            ...
            $_SESSION['info']['pets'][$index] = array();
            $_SESSION['info']['client'] = $pet['OwnedBy'];
            $_SESSION['info']['pets'][$index]['Time'] = json_decode($pet['Time'], true);
            $_SESSION['info']['pets'][$index]['Size'] = $res['Size'];
            $_SESSION['info']['pets'][$index]['GroomPrice'] = $res['GroomPrice'];
            $_SESSION['info']['pets'][$index]['BathPrice'] = $res['BathPrice'];
            
            $stmt = $database->query("SELECT * FROM Services");
            $services = $stmt->fetchAll();
            if(!empty($services)) {
                $_SESSION['info']['pets'][$index]['ID'] = $pet['ID'];
                ...
                echo '<h2>Select options for ' . $pet['Name'] . '</h2>';
                ...
                var price = 0;
                var groom = <?php echo $_SESSION['info']['pets'][$index]['GroomPrice']; ?>;
                var bath = <?php echo $_SESSION['info']['pets'][$index]['BathPrice']; ?>;
                var services = <?php echo json_encode($services); ?>;
                var size = "<?php echo $_SESSION['info']['pets'][$index]['Size'] ?>";
        ...
        if($_POST['package'] == 1 || $_POST['package'] == 2 || $_POST['package'] == 3 || $_POST['package'] == 4) {
            $index = sizeof($_SESSION['info']['pets']) - 1;
            $_SESSION['info']['pets'][$index]['package'] = $_POST['package'];
        }
        else {
